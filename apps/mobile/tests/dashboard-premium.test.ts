...
test('hero elimina redundância e mantém métricas livres de cards internos', async () => {
  ...
  assert.match(hero, /allowFontScaling=\{false\}/);
  assert.match(hero, />\s*R\$\s*<\/Text>/);
  assert.match(hero, /style=\{\[styles\.flowRing/);
  assert.match(hero, /style=\{styles\.flowArrowMask\}/);
  assert.match(hero, /backgroundColor: primitives\.color\.neutral\[950\]/);
  assert.match(hero, /incomeButton: \{ borderColor: tokens\.status\.positive/);
  assert.match(hero, /expenseButton: \{ borderColor: tokens\.status\.risk/);
  assert.match(hero, /heroDivider/);
  assert.doesNotMatch(hero, /trend-up|trend-down|chevron-up|chevron-down/);
