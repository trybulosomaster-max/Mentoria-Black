...
password_tests="$repo_root/supabase/tests/admin_password_management_v1_test.sql"
suffix="${BASHPID:-$$}"
database="aviora_admin_password_management_v1_${suffix}"

cleanup() {
  case "$database" in
    aviora_admin_password_management_v1_*)
...
test_output="$(psql_db < "$password_tests")"
...
if ! rg -q '^[[:space:]]*ok [0-9]+' <<<"$test_output"; then
  printf '%s\n' "$test_output" >&2
  echo "admin password management pgTAP produced no successful assertions" >&2
  exit 1
fi

assertions="$(rg -c '^[[:space:]]*ok [0-9]+' <<<"$test_output")"
echo "AVIORA admin password management: ${assertions} pgTAP assertions passed in an isolated local database"
