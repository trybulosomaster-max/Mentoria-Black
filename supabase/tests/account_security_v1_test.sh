...
security_tests="$repo_root/supabase/tests/account_security_v1_test.sql"
suffix="${BASHPID:-$$}"
database="aviora_account_security_v1_${suffix}"

cleanup() {
  case "$database" in
    aviora_account_security_v1_*)
...
test_output="$(psql_admin_db < "$security_tests")"
...
if ! rg -q '^[[:space:]]*ok [0-9]+' <<<"$test_output"; then
  printf '%s\n' "$test_output" >&2
  echo "account security pgTAP produced no successful assertions" >&2
  exit 1
fi

assertions="$(rg -c '^[[:space:]]*ok [0-9]+' <<<"$test_output")"
echo "AVIORA account security: ${assertions} pgTAP assertions passed in an isolated local database"
