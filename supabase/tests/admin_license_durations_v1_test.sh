...
duration_tests="$repo_root/supabase/tests/admin_license_durations_v1_test.sql"
suffix="${BASHPID:-$$}"
database="aviora_admin_license_durations_v1_${suffix}"

cleanup() {
  case "$database" in
    aviora_admin_license_durations_v1_*)
...
test_output="$(psql_db < "$duration_tests")"
...
if ! rg -q '^[[:space:]]*ok [0-9]+' <<<"$test_output"; then
  printf '%s\n' "$test_output" >&2
  echo "admin license durations pgTAP produced no successful assertions" >&2
  exit 1
fi

assertions="$(rg -c '^[[:space:]]*ok [0-9]+' <<<"$test_output")"
echo "AVIORA admin license durations: ${assertions} pgTAP assertions passed in an isolated local database"
